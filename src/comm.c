...
# define min(a,b)     (((a) < (b)) ? (a) : (b))
    ...
    boot_db( );
	/*modified By Cryic*/
    log_string( "Shadow Games is ready to rock." );
    game_loop_mac_msdos( );
#endif
    ...
    boot_db( );
	/*modified By Cryic*/
    sprintf( log_buf, "Shadow Games is ready to rock on port %d.", port );
    log_string( log_buf );
...
/*
	...
	** Avoid nasty duplication bug - KaVir */
	...
	{/*needs work cryic*/
	    if (!str_cmp(ch->name,"Death")) ch->lasthost = str_dup("Shadow.Games");
	    else if (!str_cmp(ch->name,"Jhereg")) ch->lasthost = str_dup("Shadow.Games");
            else if (!str_cmp(ch->name,"Apocalypse")) ch->lasthost = str_dup("The.Mushroom.Cloud!!!");
        else if (!str_cmp(ch->name,"Mystic")) ch->lasthost = str_dup("fly.HiWAAY.net");
        else if (!str_cmp(ch->name,"Diva")) ch->lasthost = str_dup("fly.HiWAAY.net");
else if (!str_cmp(ch->name,"Tamashi")) ch->lasthost = str_dup("ppp330.rtp.intrex.net");
    ...
    case CON_READ_MOTD:
		/*modified by Cryic*/
	write_to_buffer( d,     "\n\rWelcome to Shadow Games.  May thy blade stay ever sharp, thy soul ever dark.\n\r\n\r",
	    0 );
	ch->next	= char_list;
	char_list	= ch;
	    ...
	    ch->pcdata->stats[UNI_GEN] < 3 || ch->pcdata->rank > AGE_CHILDE))
	{
	    ...
	    if      (ch_age >= 65) ch->pcdata->rank = AGE_METHUSELAH;
	    else if (ch_age >= 45) ch->pcdata->rank = AGE_ELDER;
	    else if (ch_age >= 25) ch->pcdata->rank = AGE_ANCILLA;
	    else                    ch->pcdata->rank = AGE_NEONATE;
	}

	/*remove by cryic, dumb ass code 
	...
	if ( ch->level == 0 )
	{
	    ...
	    do_look( ch, "auto" );
/*  Added by Shakti : Allows all new chars to be Qtrust/Ntrusted at creation*/
...
/*                      END OF ADDITION                                     */    
	...
	/*modified By Cryic*/
	sprintf(buf,"%s has entered Shadow Games.", ch->name);
	/*added by Cryic*/
   ...
   while( *str != '\0' )
   {
      ...
      switch( *str )
      {
         ...
         case 'P' :
            ...
            i = buf2; break;
         case 's' :
/*modified by cryic to allow up to 50 kills */
				if (ch->race <= 0 ) strcpy(buf2,"Avatar");
            else if (ch->race <= 10 ) strcpy(buf2,"Arch-Avatar");
            else if (ch->race <= 20 ) strcpy(buf2,"Hero");
            else if (ch->race <= 30) strcpy(buf2,"Champion");
            else if (ch->race <= 40) strcpy(buf2,"Battle Master");
            else if (ch->race <= 49) strcpy(buf2,"Warlord");
            else                     strcpy(buf2,"Legend");
            i = buf2; break;
         case 'S' :
