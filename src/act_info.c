...
char *format_obj_to_char( OBJ_DATA *obj, CHAR_DATA *ch, bool fShort )
{
    ...
    if ( IS_SET(obj->quest, QUEST_ARTIFACT)) 
	{
		...
		strcat(buf, "{Relic} " );
			/*added by Cryic*/
		ADD_COLOUR(ch,buf,D_RED);
		}
	/*New Item Names And a few color changes by Death*/
    else if ( obj->points > 1199 ) 
		{
		strcat(buf, "<-Mythical-> " );
		ADD_COLOUR(ch,buf,D_BLUE);
		}
	else if ( obj->points > 599 )
	    {
		strcat(buf, "(-Legendary-) " );
		ADD_COLOUR(ch,buf,L_BLUE);
	    }
	else if ( obj->points > 50 )
	    {
		strcat(buf, "(Enhanced) " );
...
void show_char_to_char_1( CHAR_DATA *victim, CHAR_DATA *ch )
{
    ...
    if (!IS_NPC( victim ))
    {
    	...
    	{
		if (IS_DEMAFF(victim, DEM_HORNS)) act("$N has nine inch obsidian horns extending from $S head.",ch,NULL,victim,TO_CHAR);
...
/*modified to accept status kills up to 50 by Cryic*/
	...
	else if (!IS_NPC(ch) && ch->race <= 5) 
	{
		if IS_CLASS(ch,CLASS_VAMPIRE) send_to_char( "an Undead.\n\r", ch);
		else if IS_CLASS(ch,CLASS_HIGHLANDER) send_to_char( "an Initiate.\n\r", ch);
		else if IS_CLASS(ch,CLASS_MAGE) send_to_char( "a Chanter.\n\r", ch);
		else if IS_CLASS(ch,CLASS_WEREWOLF) send_to_char( "a Wolfen.\n\r", ch);
		else if IS_CLASS(ch,CLASS_DEMON) send_to_char( "a Lost Soul.\n\r", ch);
		else send_to_char( "an Arch-Avatar.\n\r", ch);
	}
	else if (!IS_NPC(ch) && ch->race <= 10) 
	{
		if IS_CLASS(ch,CLASS_VAMPIRE) send_to_char( "a Ghoul.\n\r", ch);
		else if IS_CLASS(ch,CLASS_HIGHLANDER) send_to_char( "a Duelist.\n\r", ch);
		else if IS_CLASS(ch,CLASS_MAGE) send_to_char( "a Dawnbringer.\n\r", ch);
		else if IS_CLASS(ch,CLASS_WEREWOLF) send_to_char( "a Stalker.\n\r", ch);
		else if IS_CLASS(ch,CLASS_DEMON) send_to_char( "a Lesser Shadow.\n\r", ch);
		else send_to_char( "a Arch-Avatar.\n\r", ch);
	}
	else if (!IS_NPC(ch) && ch->race <= 15) 
	{
		if IS_CLASS(ch,CLASS_VAMPIRE) send_to_char( "a Lesser Fiend.\n\r", ch);
		else if IS_CLASS(ch,CLASS_HIGHLANDER) send_to_char( "a Brother.\n\r", ch);
		else if IS_CLASS(ch,CLASS_MAGE) send_to_char( "an Eldritch Hero.\n\r", ch);
		else if IS_CLASS(ch,CLASS_WEREWOLF) send_to_char( "a Hunter.\n\r", ch);
		else if IS_CLASS(ch,CLASS_DEMON) send_to_char( "a Shadow.\n\r", ch);
		else send_to_char( "a Hero.\n\r", ch);
	}
	else if (!IS_NPC(ch) && ch->race <= 20) 
	{
		if IS_CLASS(ch,CLASS_VAMPIRE) send_to_char( "a Fiend.\n\r", ch);
		else if IS_CLASS(ch,CLASS_HIGHLANDER) send_to_char( "an Acolyte.\n\r", ch);
		else if IS_CLASS(ch,CLASS_MAGE) send_to_char( "a Magus.\n\r", ch);
		else if IS_CLASS(ch,CLASS_WEREWOLF) send_to_char( "a Predator.\n\r", ch);
		else if IS_CLASS(ch,CLASS_DEMON) send_to_char( "a ShadowLord.\n\r", ch);
		else send_to_char( "a Hero.\n\r", ch);
	}
	else if (!IS_NPC(ch) && ch->race <= 25) 
	{
		if IS_CLASS(ch,CLASS_VAMPIRE) send_to_char( "an Arch Fiend.\n\r", ch);
		else if IS_CLASS(ch,CLASS_HIGHLANDER) send_to_char( "a Wanderer.\n\r", ch);
		else if IS_CLASS(ch,CLASS_MAGE) send_to_char( "a Sorcerer.\n\r", ch);
		else if IS_CLASS(ch,CLASS_WEREWOLF) send_to_char( "a Howler.\n\r", ch);
		else if IS_CLASS(ch,CLASS_DEMON) send_to_char( "a Lesser Devil.\n\r", ch);
		else send_to_char( "a Champion.\n\r", ch);
	}
	else if (!IS_NPC(ch) && ch->race <= 30) 
	{
		if IS_CLASS(ch,CLASS_VAMPIRE) send_to_char( "a Darkwalker.\n\r", ch);
		else if IS_CLASS(ch,CLASS_HIGHLANDER) send_to_char( "a North Wind.\n\r", ch);
		else if IS_CLASS(ch,CLASS_MAGE) send_to_char( "a Wizard.\n\r", ch);
		else if IS_CLASS(ch,CLASS_WEREWOLF) send_to_char( "a Nightwalker.\n\r", ch);
		else if IS_CLASS(ch,CLASS_DEMON) send_to_char( "a Devil.\n\r", ch);
		else send_to_char( "a Champion.\n\r", ch);
	}
	else if (!IS_NPC(ch) && ch->race <= 35) 
	{
		if IS_CLASS(ch,CLASS_VAMPIRE) send_to_char( "a Blood Servant.\n\r", ch);
		else if IS_CLASS(ch,CLASS_HIGHLANDER) send_to_char( "a South Wind.\n\r", ch);
		else if IS_CLASS(ch,CLASS_MAGE) send_to_char( "a Mystic Knight.\n\r", ch);
		else if IS_CLASS(ch,CLASS_WEREWOLF) send_to_char( "a Wolverine.\n\r", ch);
		else if IS_CLASS(ch,CLASS_DEMON) send_to_char( "an Arch Devil.\n\r", ch);
		else send_to_char( "a Battle Master.\n\r", ch);
	}
	else if (!IS_NPC(ch) && ch->race <= 40) 
	{
		if IS_CLASS(ch,CLASS_VAMPIRE) send_to_char( "a Blood Warrior.\n\r", ch);
		else if IS_CLASS(ch,CLASS_HIGHLANDER) send_to_char( "a West Wind.\n\r", ch);
		else if IS_CLASS(ch,CLASS_MAGE) send_to_char( "a War Mage.\n\r", ch);
		else if IS_CLASS(ch,CLASS_WEREWOLF) send_to_char( "a Garou.\n\r", ch);
		else if IS_CLASS(ch,CLASS_DEMON) send_to_char( "a Chaos Knight.\n\r", ch);
		else send_to_char( "a Battle Master.\n\r", ch);
	}
	else if (!IS_NPC(ch) && ch->race <= 45) 
	{
		if IS_CLASS(ch,CLASS_VAMPIRE) send_to_char( "a Blood Knight.\n\r", ch);
		else if IS_CLASS(ch,CLASS_HIGHLANDER) send_to_char( "a East Wind.\n\r", ch);
		else if IS_CLASS(ch,CLASS_MAGE) send_to_char( "a Battle Mage.\n\r", ch);
		else if IS_CLASS(ch,CLASS_WEREWOLF) send_to_char( "The Crimson Claw.\n\r", ch);
		else if IS_CLASS(ch,CLASS_DEMON) send_to_char( "a Chaos Lord.\n\r", ch);
		else send_to_char( "a Warlord.\n\r", ch);
	}
	else if (!IS_NPC(ch) && ch->race <= 49) 
	{
		if IS_CLASS(ch,CLASS_VAMPIRE) send_to_char( "a Crimson Lord.\n\r", ch);
		else if IS_CLASS(ch,CLASS_HIGHLANDER) send_to_char( "a Sword Master.\n\r", ch);
		else if IS_CLASS(ch,CLASS_MAGE) send_to_char( "a Mage Lord.\n\r", ch);
		else if IS_CLASS(ch,CLASS_WEREWOLF) send_to_char( "The Crimson Fang.\n\r", ch);
		else if IS_CLASS(ch,CLASS_DEMON) send_to_char( "a Lord of Hell.\n\r", ch);
		else send_to_char( "a Warlord.\n\r", ch);
	}
	else if (!IS_NPC(ch) && ch->race == 50) 
	{
		if IS_CLASS(ch,CLASS_VAMPIRE) send_to_char( "the Crimson Doom.\n\r", ch);
		else if IS_CLASS(ch,CLASS_HIGHLANDER) send_to_char( "the Grandfather.\n\r", ch);
		else if IS_CLASS(ch,CLASS_MAGE) send_to_char( "the Chosen.\n\r", ch);
		else if IS_CLASS(ch,CLASS_WEREWOLF) send_to_char( "the Forvalaka.\n\r", ch);
		else if IS_CLASS(ch,CLASS_DEMON) send_to_char( "the Widowmaker.\n\r", ch);
		else send_to_char( "a Legend.\n\r", ch);
	}
...
void do_time( CHAR_DATA *ch, char *argument )
{
	...
	/*modified by Cryic */
    sprintf( buf,
	"It is %d o'clock %s, Day of %s, %d%s the Month of %s.\n\rShadow Games started up at %s\rThe system time is %s\r",
...
void do_weather( CHAR_DATA *ch, char *argument )
{
    ...
    /*commetted out by ryan, only one parater
	act_info.c:1682: macro `IS_SET' used with just one arg
...
void do_who( CHAR_DATA *ch, char *argument )
{
	...
	default: break;
	case MAX_LEVEL -  0:
		/*commeted out by Cryic, not sure if we need names */
	     if (!str_cmp(wch->name,"Jhereg")) class =  "The Coder    ";
    else if (!str_cmp(wch->name,"Apocalypse")) class = "The End      ";
	else if (!str_cmp(wch->name,"Death")) class =   "Oblivion     ";
...
*/
/*modified by cryic to allow 50 status kills */
		if ( IS_CLASS(wch, CLASS_DEMON)) /* || IS_SET(wch->act, PLR_CHAMPION))*/
		{
		     if (wch->race <= 0 ) class =  "Avatar       ";
		else if (wch->race <= 5 ) class =  "Lost Soul    ";
		else if (wch->race <= 10) class =  "Lesser Shadow";
		else if (wch->race <= 15) class =  "Shadow       ";
		else if (wch->race <= 20) class =  "ShadowLord   ";
		else if (wch->race <= 25) class =  "Lesser Devil ";
		else if (wch->race <= 30) class =  "Devil        ";
		else if (wch->race <= 35) class =  "Arch Devil   ";
		else if (wch->race <= 40) class =  "Chaos Knight ";
		else if (wch->race <= 45) class =  "Chaos Lord   ";
		else if (wch->race <= 49) class =  "Lord of Hell ";
		                      else class = "Widowmaker   ";
		}
		else if ( IS_CLASS(wch, CLASS_VAMPIRE))
		{
		     if (wch->race <= 0 ) class =  "Avatar       ";
		else if (wch->race <= 5 ) class =  "Undead       ";
		else if (wch->race <= 10) class =  "Ghoul        ";
		else if (wch->race <= 15) class =  "Lesser Fiend ";
		else if (wch->race <= 20) class =  "Fiend        ";
		else if (wch->race <= 25) class =  "Arch    Fiend";
		else if (wch->race <= 30) class =  "Darkwalker   ";
		else if (wch->race <= 35) class =  "Blood Servant";
		else if (wch->race <= 40) class =  "Blood Warrior";
		else if (wch->race <= 45) class =  "Blood Knight ";
		else if (wch->race <= 49) class =  "Crimson Lord ";
		                      else class = "Crimson Doom ";
		
		}
		else if ( IS_CLASS(wch, CLASS_WEREWOLF))
		{
		     if (wch->race <= 0 ) class =  "Avatar       ";
		else if (wch->race <= 5 ) class =  "Wolfen       ";
		else if (wch->race <= 10) class =  "Stalker      ";
		else if (wch->race <= 15) class =  "Hunter       ";
		else if (wch->race <= 20) class =  "Predator     ";
		else if (wch->race <= 25) class =  "Howler       ";
		else if (wch->race <= 30) class =  "Nightwalker  ";
		else if (wch->race <= 35) class =  "Wolverine    ";
		else if (wch->race <= 40) class =  "Garou        ";
		else if (wch->race <= 45) class =  "Crimson Claw ";
		else if (wch->race <= 49) class =  "Crimson Fang ";
		                      else class = "Forvalaka    ";
		...
		else if (wch->race <= 10) class =  "Eldritch Hero";
		else if (wch->race <= 15) class =  "Mystic Knight";
		else if (wch->race <= 20) class =  "Magus        ";
		else if (wch->race <= 25) class =  "Sorcerer     ";
		else if (wch->race <= 30) class =  "Wizard       ";
		else if (wch->race <= 35) class =  "Arcane Knight";
		...
		{
		     if (wch->race <= 0 ) class =  "Avatar       ";
		else if (wch->race <= 5 ) class =  "Initiate     ";
		else if (wch->race <= 10) class =  "Duelist      ";
		else if (wch->race <= 15) class =  "Brother      ";
		else if (wch->race <= 20) class =  "Acolyte      ";
		else if (wch->race <= 25) class =  "Wanderer     ";
		else if (wch->race <= 30) class =  "North Wind   ";
		else if (wch->race <= 35) class =  "South Wind   ";
		else if (wch->race <= 40) class =  "West Wind    ";
		else if (wch->race <= 45) class =  "East Wind    ";
		else if (wch->race <= 49) class =  "Sword Master ";
		                      else class = "Grandfather  ";
		...
		{
		     if (wch->race <= 0 ) class =  "Avatar       ";
		else if (wch->race <= 10 ) class = "Arch-Avatar  ";
		else if (wch->race <= 20) class =  "Hero         ";
		else if (wch->race <= 30) class =  "Champion     ";
		else if (wch->race <= 40) class =  "BattleMaster ";
		else if (wch->race <= 49) class =  "Warlord      ";
		                      else class = "Legend       ";
		}
	...
	if ( ch->class > 0 || IS_IMMORTAL( ch ))
	  {
	    ...
	    {
			/*changed to kensai by cryic*/
		sprintf( kav, ". %sKensai%s", openb, closeb);
	    }
	    else
			strcpy( kav, "." );
			}
...
void do_practice( CHAR_DATA *ch, char *argument )
{
  ...
  else
    {
		...
		/*added by Cryic to cut down on spam for practicing*/
		if ( ch->exp < 10000 )
		{
			send_to_char( "You do not have the 10,000 exp needed.\n\r", ch );
			return;
		}
		...
		{/*modified by Cryic to cut down on spam for practicing*/
			if (ch->pcdata->learned[sn] < SKILL_ADEPT)
			{
	    		ch->exp -= 10000; /*was 500, now 10k*/
			...
			ch->pcdata->learned[sn] = SKILL_ADEPT;
				...
				ch, NULL, skill_table[sn].name, TO_CHAR );
                         if ( (sn = skill_lookup(argument) == 116) )
                         {
                          ch->pcdata->powers[1] = 2047;
                          ch->pcdata->powers[2] = 1023;
                          ch->pcdata->powers[3] = 15;
                         }
...
void do_finger( CHAR_DATA *ch, char *argument )
{
    ...
    int  num2;
    int  extra;
	int  playerclass; /*added by cryic for new class names*/
	...
	num=fread_number( fp );
	playerclass =fread_number( fp ); /*cryic reads CLASS_Whatever*/
	...
	switch ( num )
	{
	    ...
	    case LEVEL_ARCHMAGE:
	    {
			/*modified by cryic for new class names, uses extra bit predefined.*/
		if (playerclass == CLASS_DEMON)
		{
		    if (num2 <= 0 ) send_to_char("They are an Avatar, ",ch);
			else if (num2 <= 5 ) send_to_char("They are a Lost Soul, ",ch);
			else if (num2 <= 10) send_to_char("They are a Lesser Shadow, ",ch);
			else if (num2 <= 15) send_to_char("They are a Shadow, ",ch);
			else if (num2 <= 20) send_to_char("They are a Shadow Lord, ",ch);
			else if (num2 <= 25) send_to_char("They are a Lesser Devil, ",ch);
			else if (num2 <= 30) send_to_char("They are a Devil, ",ch);
			else if (num2 <= 35) send_to_char("They are a Arch Devil, ",ch);
			else if (num2 <= 40) send_to_char("They are a Chaos Knight, ",ch);
			else if (num2 <= 45) send_to_char("They are a Chaos Lord, ",ch);
			else if (num2 <= 49) send_to_char("They are a Lord of Hell, ",ch);
			else send_to_char("They are the Widowmaker, ",ch);

		}
		else if (playerclass == CLASS_MAGE)
		{
		    if (num2 <= 0 ) send_to_char("They are an Avatar, ",ch);
			else if (num2 <= 5 ) send_to_char("They are a Chanter, ",ch);
			else if (num2 <= 10) send_to_char("They are a Dawnbringer, ",ch);
			else if (num2 <= 15) send_to_char("They are an Eldritch Hero, ",ch);
			else if (num2 <= 20) send_to_char("They are a Magus, ",ch);
			else if (num2 <= 25) send_to_char("They are a Sorcerer, ",ch);
			else if (num2 <= 30) send_to_char("They are a Wizard, ",ch);
			else if (num2 <= 35) send_to_char("They are an Arcane Knight, ",ch);
			else if (num2 <= 40) send_to_char("They are a War Mage, ",ch);
			else if (num2 <= 45) send_to_char("They are a Battle Mage, ",ch);
			else if (num2 <= 49) send_to_char("They are a Mage Lord, ",ch);
			else send_to_char("They are the Chosen, ",ch);

		}
		else if (playerclass == CLASS_WEREWOLF)
		{
		    if (num2 <= 0 ) send_to_char("They are an Avatar, ",ch);
			else if (num2 <= 5 ) send_to_char("They are a Wolfen, ",ch);
			else if (num2 <= 10) send_to_char("They are a Stalker, ",ch);
			else if (num2 <= 15) send_to_char("They are a Hunter, ",ch);
			else if (num2 <= 20) send_to_char("They are a Predator, ",ch);
			else if (num2 <= 25) send_to_char("They are a Howler, ",ch);
			else if (num2 <= 30) send_to_char("They are a Nightwalker, ",ch);
			else if (num2 <= 35) send_to_char("They are a Wolverine, ",ch);
			else if (num2 <= 40) send_to_char("They are a Garou, ",ch);
			else if (num2 <= 45) send_to_char("They are The Crimson Claw, ",ch);
			else if (num2 <= 49) send_to_char("They are The Crimson Fang, ",ch);
			else send_to_char(" They are the Forvalaka, ",ch);

		}
		else if (playerclass == CLASS_VAMPIRE)
		{
		    if (num2 <= 0 ) send_to_char("They are an Avatar, ",ch);
			else if (num2 <= 5 ) send_to_char("They are a Undead, ",ch);
			else if (num2 <= 10) send_to_char("They are a Ghoul, ",ch);
			else if (num2 <= 15) send_to_char("They are a Lesser Fiend, ",ch);
			else if (num2 <= 20) send_to_char("They are a Fiend, ",ch);
			else if (num2 <= 25) send_to_char("They are a Arch Fiend, ",ch);
			else if (num2 <= 30) send_to_char("They are a Darkwalker, ",ch);
			else if (num2 <= 35) send_to_char("They are a Blood Servant, ",ch);
			else if (num2 <= 40) send_to_char("They are a Blood Warrior, ",ch);
			else if (num2 <= 45) send_to_char("They are a Blood Knight, ",ch);
			else if (num2 <= 49) send_to_char("They are a Crimson Lord, ",ch);
			else send_to_char("They are the Crimson Doom, ",ch);

		}
		else if (playerclass == CLASS_HIGHLANDER)
		{
		    if (num2 <= 0 ) send_to_char("They are an Avatar, ",ch);
			else if (num2 <= 5 ) send_to_char("They are an Initiate, ",ch);
			else if (num2 <= 10) send_to_char("They are a Duelist, ",ch);
			else if (num2 <= 15) send_to_char("They are a Brother, ",ch);
			else if (num2 <= 20) send_to_char("They are a Acolyte, ",ch);
			else if (num2 <= 25) send_to_char("They are a Wanderer, ",ch);
			else if (num2 <= 30) send_to_char("They are a North Wind, ",ch);
			else if (num2 <= 35) send_to_char("They are a South Wind, ",ch);
			else if (num2 <= 40) send_to_char("They are a West Wind, ",ch);
			else if (num2 <= 45) send_to_char("They are a East Wind, ",ch);
			else if (num2 <= 49) send_to_char("They are a Sword Master, ",ch);
			else send_to_char("They are the Grandfather, ",ch);
		...
		{		
			if (num2 < 1)
				...
				send_to_char("They are an Arch-Avatar, ",ch);
			else if (num2 < 21)
				send_to_char("They are a Hero, ",ch);
			else if (num2 < 31)
				send_to_char("They are a Champion, ",ch);
			else if (num2 < 41)
				send_to_char("They are a BattleMaster, ",ch);
			else if (num2 < 50)
				send_to_char("They are a Warlord, ",ch);
			else
				send_to_char("They are a Legend, ",ch);
		}
		break;
	    }
	    case LEVEL_BUILDER:
