...
/* Modified by Shakti : A god should be able to freely interact with private*/
/* and solitary rooms.                                                      */
...
void do_rstat( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->in_room != location && room_is_private( location ) && (ch->level
          < MAX_LEVEL) )
    {
	send_to_char( "That room is private right now.\n\r", ch );
	return;
...
/*   Added By Shakti : Clearing stats on a charmed mobile crashed the mud   */

    if (IS_NPC(ch))
    {
        send_to_char("You cannot ClearStats on A Charmed Mobile.\n\r", ch);
        return;
    }
