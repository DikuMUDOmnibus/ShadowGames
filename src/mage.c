...
void do_chant( CHAR_DATA *ch, char *argument )
{
    ...
    {
    send_to_char("The energy summoned to cast the spell evaporates.\n\r", ch);
    return;
    }
...
void adv_spell_affect( CHAR_DATA *ch, OBJ_DATA *book, OBJ_DATA *page, char *argument)
{
    ...
    if IS_SET(ch->in_room->room_flags, ROOM_NOSPELL)
    {
    send_to_char("The magic you try to summon, evaporates..\n\r", ch);
    return;
    }
    ...
    {
       send_to_char("Casting on Gods?!? Your spell reflects off of the god, and mutates into disintegration!\n\r",ch);
       do_slay(ch,ch->name);
       return;
    }
...
/*  if (IS_IMMORTAL (victim ) && )
    {
      send_to_char("Casting on A God?!? Your spell transmutes into disintegrate and strikes you!\n\r",ch);
      do_slay(ch,ch->name);
      return;
    }
*/
